...
m1_file_to_write <- paste0(outDir,"/asso_result_m0.txt")
m2_file_to_write <- paste0(outDir,"/asso_result_m2.txt")
m2_1_file_to_write <- paste0(outDir,"/asso_result_m2_1.txt")
m2_2_file_to_write <- paste0(outDir,"/asso_result_m2_2.txt")
m2_3_file_to_write <- paste0(outDir,"/asso_result_m2_3.txt")
...
png(png_name, type=c("cairo", "cairo-png", "Xlib", "quartz"), width=width, height=height, units="in", res=720)
#  pdf(pdf_name, width = pdf_width, height=pdf_height)
#  plt <- ggplot(data=data,aes(x=group,y=y,fill=FishOil_intake,group = interaction(group,FishOil_intake)))+
...
# subset covariates
covars_all <- c("f.31.0.0", "f.21003.0.0", "sex_by_age", "PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10", "BMI_imp", "TSI_imp", "smoking_imp", "alcohol_imp", "physical_imp", "Statin", "FishOil", "OilyFish")
...
pheno.pgs.rmOutli$FOS_PGS <- pheno.pgs.rmOutli$FishOil * pheno.pgs.rmOutli$SCORE.norm
# add OFI-by-PGS interaction term
pheno.pgs.rmOutli$OFI_PGS <- pheno.pgs.rmOutli$OilyFish * pheno.pgs.rmOutli$SCORE.norm
...
dt_s1_g1 <- dt_s1[dt_s1$FishOil==1,]
dt_s1_g2 <- dt_s1[dt_s1$FishOil==0,]
# rank-based inverse normalization
dt_s1_INT <- copy(pheno.pgs.INT)
dt_s1_g1_INT <- dt_s1_INT[dt_s1_INT$FishOil==1,]
dt_s1_g2_INT <- dt_s1_INT[dt_s1_INT$FishOil==0,]

# strategy 1.1: stratify by two groups: oily fish low-intake vs high-intake; all participants
dt_s1_g3 <- dt_s1[dt_s1$OilyFish==1,]
dt_s1_g4 <- dt_s1[dt_s1$OilyFish==0,]
# rank-based inverse normalization
dt_s1_g3_INT <- dt_s1_INT[dt_s1_INT$OilyFish==1,]
dt_s1_g4_INT <- dt_s1_INT[dt_s1_INT$OilyFish==0,]
###########################
...
# model1 (w/o interaction): PUFA ~ sex (f.31.0.0) + age (f.21003.0.0) + sex * age + BMI + TSI + smoking + alcohol + physical actvity + statin use + FOS + oily fish intake
covar_m1 = c("f.31.0.0","f.21003.0.0","sex_by_age", "BMI_imp", "TSI_imp", "smoking_imp", "alcohol_imp", "physical_imp", "Statin", "PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10","FishOil", "OilyFish", "SCORE.norm")
formula_m1 <- as.formula(paste(trait_field, "~", paste(covar_m1, collapse = " + ")))

# model2 (w/ interaction): PUFA ~ sex (f.31.0.0) + age (f.21003.0.0) + PC1 ~ PC10 + sex * age + BMI + TSI + smoking + alcohol + physical actvity + statin use + FOS + oily fish intake + PGS (norm) + FOS * PGS
covar_m2 = c("f.31.0.0","f.21003.0.0","sex_by_age","BMI_imp", "TSI_imp", "smoking_imp", "alcohol_imp", "physical_imp", "Statin", "PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10", "FishOil", "OilyFish","SCORE.norm","FOS_PGS")
formula_m2 <- as.formula(paste(trait_field, "~", paste(covar_m2, collapse = " + ")))
# change interaction term to OFI * PGS
covar_m2_sens = c("f.31.0.0","f.21003.0.0","sex_by_age","BMI_imp", "TSI_imp", "smoking_imp", "alcohol_imp", "physical_imp", "Statin", "PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10", "FishOil", "OilyFish","SCORE.norm","OFI_PGS")
formula_m2_sens <- as.formula(paste(trait_field, "~", paste(covar_m2_sens, collapse = " + ")))

# model2.1 (stratified by FOS controlling for OilyFish): PUFA ~ sex (f.31.0.0) + age (f.21003.0.0) + PC1 ~ PC10 + sex * age + BMI + TSI + smoking + alcohol + physical actvity + statin use + PGS (norm) + OilyFish
covar_m2_1 = c("f.31.0.0","f.21003.0.0","sex_by_age","BMI_imp", "TSI_imp", "smoking_imp", "alcohol_imp", "physical_imp", "Statin", "PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10", "SCORE.norm", "OilyFish")
formula_m2_1 <- as.formula(paste(trait_field, "~", paste(covar_m2_1, collapse = " + ")))

# model2.2 (stratified by OilyFish controlling for FOS): PUFA ~ sex (f.31.0.0) + age (f.21003.0.0) + PC1 ~ PC10 + sex * age + BMI + TSI + smoking + alcohol + physical actvity + statin use + PGS (norm) + FishOil
covar_m2_2 = c("f.31.0.0","f.21003.0.0","sex_by_age","BMI_imp", "TSI_imp", "smoking_imp", "alcohol_imp", "physical_imp", "Statin", "PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10", "SCORE.norm", "FishOil")
formula_m2_2 <- as.formula(paste(trait_field, "~", paste(covar_m2_2, collapse = " + ")))

# model2.3 (stratified by PGS, without consider FOS*PGS interaction term): PUFA ~ sex (f.31.0.0) + age (f.21003.0.0) + PC1 ~ PC10 + sex * age + BMI + TSI + smoking + alcohol + physical actvity + statin use + FOS + oily fish intake + PGS (norm) 
covar_m2_3 = c("f.31.0.0","f.21003.0.0","sex_by_age","BMI_imp", "TSI_imp", "smoking_imp", "alcohol_imp", "physical_imp", "Statin", "FishOil", "OilyFish")
...
# model: m2; phenotype: raw; sensitivity analysis
data_assc <- subset(pheno.pgs, select=c(trait_field,covar_m2_sens))
assc_test(data_assc, y_name = trait_field, formula = formula_m2_sens, group = paste0(pop,"_all_m2_raw_sens"), file_to_write = m2_file_to_write)
# model: m2; phenotype: rank-based inverse normalization; sensitivity analysis
data_assc <- subset(pheno.pgs.INT, select=c(trait_field,covar_m2_sens))
assc_test(data_assc,  y_name = trait_field, formula = formula_m2_sens, group = paste0(pop,"_all_m2_INT_sens"), file_to_write = m2_file_to_write)

############ perform association test: subgroup (stratified by FOS, Oilyfish, or PGS groups); model2.1 - model 2.3; raw and INT phenotype
# model: m2.1; phenotype: raw; stratified by FishOil
...
# model: m2.2; phenotype: raw; stratified by OilyFish
data_assc <- subset(dt_s1_g3, select=c(trait_field,covar_m2_2))
assc_test(data_assc,  y_name = trait_field, formula = formula_m2_2, group = paste0(pop,"_high-OFI_m22_raw"), file_to_write = m2_2_file_to_write)
data_assc <- subset(dt_s1_g4, select=c(trait_field,covar_m2_2))
assc_test(data_assc,  y_name = trait_field, formula = formula_m2_2, group = paste0(pop,"_low-OFI_m22_raw"), file_to_write = m2_2_file_to_write)
# model: m22; phenotype: rank-based inverse normalization
data_assc <- subset(dt_s1_g3_INT, select=c(trait_field,covar_m2_2))
assc_test(data_assc,  y_name = trait_field, formula = formula_m2_2, group = paste0(pop,"_high-OFI_m22_INT"), file_to_write = m2_2_file_to_write)
data_assc <- subset(dt_s1_g4_INT, select=c(trait_field,covar_m2_2))
assc_test(data_assc,  y_name = trait_field, formula = formula_m2_2, group = paste0(pop,"_low-OFI_m22_INT"), file_to_write = m2_2_file_to_write)
...
df_to_plot <- subset(dt_s1, select=c("SCORE.norm", trait_field, "FishOil"))
...
# plot scatter plot: all PGS; Oily fish
df_to_plot <- subset(dt_s1, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_allPGS_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (raw); all PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: bottom 5% PGS; Fish oil
df_to_plot <- subset(dt_s2_g1, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: bottom 5% PGS; Oily fish
df_to_plot <- subset(dt_s2_g1, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_bottom5PGS_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (raw); bottom 5% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: top 5% PGS; Fish oil
df_to_plot <- subset(dt_s2_g2, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: top 5% PGS; Oily fish
df_to_plot <- subset(dt_s2_g2, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_top5PGS_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (raw); top 5% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: 0-20% PGS; Fish oil
df_to_plot <- subset(dt_s3_g1, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: 0-20% PGS; OilyFish
df_to_plot <- subset(dt_s3_g1, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_0To20PGS_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (raw); 0-20% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: 21-40% PGS: Fish oil
df_to_plot <- subset(dt_s3_g2, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: 21-40% PGS: OilyFish
df_to_plot <- subset(dt_s3_g2, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_20To40PGS_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (raw); 20-40% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: 41-60% PGS; Fish oil
df_to_plot <- subset(dt_s3_g3, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: 41-60% PGS; OilyFish
df_to_plot <- subset(dt_s3_g3, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_40To60PGS_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (raw); 40-60% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: 61-80% PGS; Fish oil
df_to_plot <- subset(dt_s3_g4, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: 61-80% PGS; OilyFish
df_to_plot <- subset(dt_s3_g4, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_60To80PGS_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (raw); 60-80% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: 81-100% PGS; Fish oil
df_to_plot <- subset(dt_s3_g5, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: 81-100% PGS; OilyFish
df_to_plot <- subset(dt_s3_g5, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_80To100PGS_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (raw); 80-100% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: all PGS; INT normalized phenotype; FishOil
df_to_plot <- subset(dt_s1_INT, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: all PGS; INT normalized phenotype; OilyFish
df_to_plot <- subset(dt_s1_INT, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_allPGS_INT_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (INT); all PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: bottom 5% PGS; INT normalized phenotype; FishOil
df_to_plot <- subset(dt_s2_g1_INT, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: bottom 5% PGS; INT normalized phenotype; OilyFish
df_to_plot <- subset(dt_s2_g1_INT, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_bottom5PGS_INT_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (INT); bottom 5% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: top 5% PGS; INT normalized phenotype; FishOil
df_to_plot <- subset(dt_s2_g2_INT, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: top 5% PGS; INT normalized phenotype; OilyFish
df_to_plot <- subset(dt_s2_g2_INT, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_top5PGS_INT_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (INT); top 5% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: 0-20% PGS; INT normalized phenotype; FishOil
df_to_plot <- subset(dt_s3_g1_INT, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: 0-20% PGS; INT normalized phenotype; OilyFish
df_to_plot <- subset(dt_s3_g1_INT, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_0To20PGS_INT_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (INT); 0-20% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: 21-40% PGS; INT normalized phenotype; FishOil
df_to_plot <- subset(dt_s3_g2_INT, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: 21-40% PGS; INT normalized phenotype; OilyFish
df_to_plot <- subset(dt_s3_g2_INT, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_20To40PGS_INT_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (INT); 20-40% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: 41-60% PGS; INT normalized phenotype; FishOil
df_to_plot <- subset(dt_s3_g3_INT, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: 41-60% PGS; INT normalized phenotype; OilyFish
df_to_plot <- subset(dt_s3_g3_INT, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_40To60PGS_INT_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (INT); 40-60% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: 61-80% PGS; INT normalized phenotype; FishOil
df_to_plot <- subset(dt_s3_g4_INT, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: 61-80% PGS; INT normalized phenotype; OilyFish
df_to_plot <- subset(dt_s3_g4_INT, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_60To80PGS_INT_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (INT); 60-80% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

# plot scatter plot: 81-100% PGS; INT normalized phenotype; FishOil
df_to_plot <- subset(dt_s3_g5_INT, select=c("SCORE.norm", trait_field, "FishOil"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "FOS"
df_to_plot$group[df_to_plot$group==0] <- "non-FOS"
...
# plot scatter plot: 81-100% PGS; INT normalized phenotype; OilyFish
df_to_plot <- subset(dt_s3_g5_INT, select=c("SCORE.norm", trait_field, "OilyFish"))
colnames(df_to_plot) <- c("x", "y", "group")
df_to_plot$group[df_to_plot$group==1] <- "high-OFI"
df_to_plot$group[df_to_plot$group==0] <- "low-OFI"
plot_scatter2(png_name=paste0(outDir,"/scatter_strat_80To100PGS_INT_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (INT); 80-100% PGS groups; OFI"),x_name = 'PGS (scaled)',y_name = trait_name,width = 6, height = 5)

####################
# plot boxplot: PGS percentiles; raw phenotype; Fish oil
df_to_plot <- rbind(subset(dt_s2_g1,select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
		    subset(dt_s2_g2,select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
		    subset(dt_s3_g1, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
		    subset(dt_s3_g2, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
		    subset(dt_s3_g3, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
		    subset(dt_s3_g4, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
		    subset(dt_s3_g5, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")))
plot_dt_s1 <- copy(dt_s1); plot_dt_s1$pgs_perct <- "All"
df_to_plot <- rbind(df_to_plot, subset(plot_dt_s1, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")))
...
# plot boxplot: PGS percentiles; raw phenotype; Oily fish 
df_to_plot <- rbind(subset(dt_s2_g1,select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s2_g2,select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s3_g1, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s3_g2, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s3_g3, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s3_g4, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s3_g5, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")))
plot_dt_s1 <- copy(dt_s1); plot_dt_s1$pgs_perct <- "All"
df_to_plot <- rbind(df_to_plot, subset(plot_dt_s1, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")))
df_to_plot$pgs_perct <- factor(df_to_plot$pgs_perct, levels=c("0-5%","0-20%","20-40%","40-60%","60-80%","80-100%","95-100%","All"))
colnames(df_to_plot) <- c("x", "y", "fo_group", "pgs_group")
df_to_plot$fo_group[df_to_plot$fo_group==1] <- "high-OFI"
df_to_plot$fo_group[df_to_plot$fo_group==0] <- "low-OFI"
df_to_plot <- df_to_plot[!is.na(df_to_plot$fo_group),]
plot_boxplot(png_name=paste0(outDir,"/boxplot_strat_PGSgroups_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (raw); all PGS groups; OFI"), y_name = trait_name,width = 8, height = 6)

# plot boxplot: PGS percentiles; INT phenotype; Fish oil
df_to_plot <- rbind(subset(dt_s2_g1_INT,select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
                    subset(dt_s2_g2_INT,select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
                    subset(dt_s3_g1_INT, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
                    subset(dt_s3_g2_INT, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
                    subset(dt_s3_g3_INT, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
                    subset(dt_s3_g4_INT, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")),
                    subset(dt_s3_g5_INT, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")))
plot_dt_s1_INT <- copy(dt_s1_INT); plot_dt_s1_INT$pgs_perct <- "All"
df_to_plot <- rbind(df_to_plot, subset(plot_dt_s1_INT, select=c("SCORE.norm", trait_field, "FishOil", "pgs_perct")))
df_to_plot$pgs_perct <- factor(df_to_plot$pgs_perct, levels=c("0-5%","0-20%","20-40%","40-60%","60-80%","80-100%","95-100%","All"))
colnames(df_to_plot) <- c("x", "y", "fo_group", "pgs_group")
df_to_plot$fo_group[df_to_plot$fo_group==1] <- "FOS"
df_to_plot$fo_group[df_to_plot$fo_group==0] <- "non-FOS"
...
# plot boxplot: PGS percentiles; INT phenotype; Oily fish
df_to_plot <- rbind(subset(dt_s2_g1_INT,select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s2_g2_INT,select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s3_g1_INT, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s3_g2_INT, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s3_g3_INT, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s3_g4_INT, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")),
                    subset(dt_s3_g5_INT, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")))
plot_dt_s1_INT <- copy(dt_s1_INT); plot_dt_s1_INT$pgs_perct <- "All"
df_to_plot <- rbind(df_to_plot, subset(plot_dt_s1_INT, select=c("SCORE.norm", trait_field, "OilyFish", "pgs_perct")))
df_to_plot$pgs_perct <- factor(df_to_plot$pgs_perct, levels=c("0-5%","0-20%","20-40%","40-60%","60-80%","80-100%","95-100%","All"))
colnames(df_to_plot) <- c("x", "y", "fo_group", "pgs_group")
df_to_plot$fo_group[df_to_plot$fo_group==1] <- "high-OFI"
df_to_plot$fo_group[df_to_plot$fo_group==0] <- "low-OFI"
df_to_plot <- df_to_plot[!is.na(df_to_plot$fo_group),]
plot_boxplot(png_name=paste0(outDir,"/boxplot_strat_PGSgroups_INT_OFI_",trait_name,".png"),data = df_to_plot,title = paste0(trait_name," (INT); all PGS groups; OFI"), y_name = trait_name,width = 8, height = 6)
